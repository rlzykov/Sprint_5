from logindata import UserProfile, RandomData
...
class TestRegistrationPage:
    ...
    def test_registration_success(self, driver):
        ...
        driver = driver
        driver.get(LinksStorage.REG_PAGE_URL)
        ...
        assert driver.current_url == LinksStorage.AUTH_PAGE_URL and login_btn_displayed
    ...
    def test_registration_incorrect_password_check_error(self, driver):
        ...
        driver.get(LinksStorage.REG_PAGE_URL)
        WebDriverWait(driver, 5).until(EC.visibility_of_element_located(RegistrationPageLocators.registration_btn))
        driver.find_element(*RegistrationPageLocators.name_input).send_keys(UserProfile.user_name)
        driver.find_element(*RegistrationPageLocators.email_input).send_keys(UserProfile.email)
        ...
        assert (error == 'Некорректный пароль') and (driver.current_url == LinksStorage.REG_PAGE_URL)
    ...
    def test_double_registration_check_error(self, driver):
        ...
        driver.find_element(*RegistrationPageLocators.name_input).send_keys(UserProfile.user_name)
        driver.find_element(*RegistrationPageLocators.email_input).send_keys(UserProfile.email)
        driver.find_element(*RegistrationPageLocators.password_input).send_keys(UserProfile.password)
        driver.find_element(*RegistrationPageLocators.registration_btn).click()
        WebDriverWait(driver, 3).until(
        ...
        assert (error == 'Такой пользователь уже существует') and (driver.current_url == LinksStorage.REG_PAGE_URL)
